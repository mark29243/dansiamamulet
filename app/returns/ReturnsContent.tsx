  ...
  const content = {
    th: {
      ...
      returns: {
        ...
        items: [
          { icon: '📅', title: 'ระยะเวลา', text: 'แจ้งคืนภายใน 7 วันหลังได้รับสินค้า' },
          { icon: '✅', title: 'เงื่อนไขสินค้า', text: 'สินค้าต้องอยู่ในสภาพเดิม ไม่มีร่องรอยการใช้งาน ครบพร้อมใบรับรอง' },
          { icon: '🔍', title: 'เหตุผล', text: 'รับคืนเฉพาะกรณีที่พิสูจน์ได้ว่าสินค้าไม่แท้เท่านั้น' },
          { icon: '💰', title: 'การคืนเงิน', text: 'คืนเงินเต็มจำนวนภายใน 5-7 วันทำการ หลังได้รับสินค้าคืน' },
          { icon: '📦', title: 'ค่าส่งคืน', text: 'ลูกค้าเป็นผู้รับผิดชอบค่าส่งสินค้าคืน ยกเว้นกรณีที่ร้านส่งผิด' },
        ],
      },
      process: {
    ...
    en: {
      ...
      returns: {
        title: '↩ Return Policy',
        items: [
          { icon: '📅', title: 'Time Limit', text: 'Contact us within 7 days of receiving your order' },
          { icon: '✅', title: 'Condition', text: 'Item must be in original condition with no signs of use, including certificate' },
          { icon: '🔍', title: 'Reason', text: 'Returns accepted only if the item is proven to be inauthentic' },
          { icon: '💰', title: 'Refund', text: 'Full refund within 5-7 business days after we receive the item back' },
          { icon: '📦', title: 'Return Shipping', text: 'Buyer covers return shipping costs, unless the wrong item was sent' },
        ],
      },
      process: {
    ...
    zh: {
      ...
      returns: {
        title: '↩ 退货政策',
        items: [
          { icon: '📅', title: '时限', text: '收到商品后7天内联系我们' },
          { icon: '✅', title: '商品状态', text: '商品必须保持原始状态，无使用痕迹，附真品证书' },
          { icon: '🔍', title: '原因', text: '仅接受经证明为非正品的退货' },
          { icon: '💰', title: '退款', text: '收到退货后5-7个工作日内全额退款' },
          { icon: '📦', title: '退货运费', text: '买家承担退货运费，除非发错商品' },
        ],
      },
      process: {
        title: '📋 退货流程',
  ...
  return (
    <div className="container animate-fade-in" style={{ padding: '40px 24px 80px', maxWidth: 800 }}>
      ...
      <div className="card" style={{ padding: 28 }}>
        <h2 className="serif" style={{ fontSize: 18, fontWeight: 600, marginBottom: 20 }}>{t.process.title}</h2>
