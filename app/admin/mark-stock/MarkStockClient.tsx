...
export default function MarkStockClient({ initialProducts }: { initialProducts: any[] }) {
  ...
  return (
    <div style={{ maxWidth: 600, margin: '0 auto', padding: '16px', fontFamily: 'sans-serif' }}>
      ...
      <div style={{ marginBottom: 20 }}>
        <h1 style={{ fontSize: 22, fontWeight: 'bold', margin: 0, color: '#111' }}>เช็คสต็อกส่วนตัว (Mark's Stock)</h1>
        <p style={{ fontSize: 13, color: '#666', margin: '4px 0 0 0' }}>Dansiamamulets — ดึงข้อมูลเฉพาะของคุณมาร์ค</p>
      </div>

      <button onClick={handleMigrate} style={{ width: '100%', padding: '10px', background: '#92400e', color: 'white', border: 'none', borderRadius: 8, marginBottom: 20, fontWeight: 'bold' }}>
        📥 ดึงข้อมูลล่าสุดจาก Google Sheets (MARK)
      </button>
          ...
          return (
            <div key={p.id} style={{ background: 'white', borderRadius: 12, padding: 12, border: '1px solid #eaeaea', display: 'flex', flexDirection: 'column', gap: 12 }}>
              ...
              <div style={{ display: 'flex', gap: 12 }}>
                <div style={{ width: 80, height: 80, borderRadius: 8, overflow: 'hidden', position: 'relative', flexShrink: 0, border: '1px solid #f0f0f0' }}>
