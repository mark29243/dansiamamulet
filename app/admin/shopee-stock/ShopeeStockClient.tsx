...
export default function ShopeeStockClient({ initialProducts, isStaffRoute = false }: { initialProducts: any[], isStaffRoute?: boolean }) {
  ...
  const handleClearEmpty = async () => {
    ...
    setLoading(true);
    try {
      const res = await fetch('/api/admin/shopee-stock/clear-empty');
  ...
  const countShopee = products.filter(p => p.name_shopee && p.name_shopee.trim() !== '' && !p.mark_shopee2).length;
  const countThaimart = products.filter(p => p.mark_thaimart).length;
  const count1Pic = products.filter(p => p.images && p.images.length === 1).length;
  ...
  const filteredProducts = useMemo(() => {
    ...
    if (activeFilter === 'SOLD') {
      ...
      if (activeFilter === 'THAIMART') list = list.filter(p => p.mark_thaimart);
      if (activeFilter === 'NOLOC') list = list.filter(p => !p.mark_location || p.mark_location.trim() === '');
      if (activeFilter === '1_PIC') list = list.filter(p => p.images && p.images.length === 1);
    }
  ...
  const handleSingleImport = async () => {
    ...
    try {
      ...
      // 2. Process & Save to shopee_products table
  ...
  const handleExportExcel = () => {
    ...
    const data = filteredProducts.map(p => ({
      'รหัสสินค้า': p.name_shopee && !p.mark_shopee2 ? p.name_shopee : '',
      'ชื่อสินค้า': p.name,
      'ราคา': p.price || 0,
      'สต็อก': p.stock || 0,
      'สถานที่จัดเก็บ': p.mark_location || '',
      'Shopee 1': (p.name_shopee && p.name_shopee.trim() !== '' && !p.mark_shopee2) ? '✓' : '',
      'Shopee 2': p.mark_shopee2 ? '✓' : '',
      'Facebook': p.mark_fb ? '✓' : '',
      'Instagram': p.mark_ig ? '✓' : '',
      'Tiktok': p.mark_tt ? '✓' : '',
      'Thaimart': p.mark_thaimart ? '✓' : '',
    ...
    XLSX.writeFile(workbook, `Mark_Stock_${new Date().toISOString().split('T')[0]}.xlsx`);
  };
  ...
  const handleExportShopeeTemplate = async () => {
    ...
    try {
      ...
      XLSX.writeFile(wb, `Mark_Shopee_MassUpload_${new Date().toISOString().split('T')[0]}.xlsx`);
    } catch (error: any) {
  ...
  return (
    <div style={{ maxWidth: 600, margin: '0 auto', padding: '16px', fontFamily: 'sans-serif' }}>
      ...
      <div style={{ background: '#FFF3E0', padding: 16, borderRadius: 10, border: '1px solid #FFCC80', marginBottom: 20 }}>
        ...
        <div style={{ display: 'flex', gap: 8 }}>
          <input
            ...
            style={{ flex: 1, padding: '10px 12px', fontSize: 13, border: '1px solid #FFCC80', borderRadius: 6, minWidth: 0 }}
          />
          <button
            onClick={handleSingleImport}
            disabled={isImporting || !shopeeUrl}
            style={{ 
              padding: '10px 16px', background: '#F97316', border: 'none', color: '#fff', 
              borderRadius: 6, fontSize: 13, fontWeight: 'bold', cursor: (isImporting || !shopeeUrl) ? 'not-allowed' : 'pointer',
              whiteSpace: 'nowrap'
            }}
          >
        ...
        <div style={{ fontSize: 11, color: '#F97316', marginTop: 6, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <span>* ระบบจะดึงรูปภาพ, ชื่อ, ราคา และบันทึกลงในรายการสต็อกให้อัตโนมัติ</span>
          <button 
            onClick={() => setShowManual(!showManual)}
            style={{ background: 'none', border: 'none', color: '#D97706', textDecoration: 'underline', fontSize: 12, cursor: 'pointer', padding: 0 }}
          >
            {showManual ? 'ซ่อนการเพิ่มแมนนวล' : 'ดึงไม่ผ่าน? เพิ่มข้อมูลเอง (Manual)'}
          </button>
        </div>
      ...
      <div style={{ display: 'flex', gap: 12, marginBottom: 20 }}>
        ...
        <button onClick={() => window.location.href='/staff/import-shopee'} style={{ flex: 1, padding: '12px', background: '#ea580c', color: 'white', border: 'none', borderRadius: 8, fontWeight: 'bold', cursor: 'pointer' }}>
          📊 อัปโหลด (Excel)
        </button>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(5, 1fr)', gap: 8, marginBottom: 20 }}>
        ...
        <StatBox label="มีแค่ 1 รูป" value={count1Pic} color="#9333ea" onClick={() => setActiveFilter('1_PIC')} active={activeFilter==='1_PIC'} />
        ...
        <StatBox label="THAIMART" value={countThaimart} onClick={() => setActiveFilter('THAIMART')} active={activeFilter==='THAIMART'} />
      </div>
            ...
            <div key={p.id} style={{ background: 'white', borderRadius: 12, padding: 12, border: '1px solid #eaeaea', display: 'flex', flexDirection: 'column', gap: 12 }}>
              ...
              <div style={{ display: 'flex', gap: 12 }}>
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginBottom: 8 }}>
                    {p.images && p.images.length === 1 && <span style={{ fontSize: 10, background: '#f3e8ff', color: '#7e22ce', padding: '2px 6px', borderRadius: 4, fontWeight: 'bold' }}>⚠️ มีรูปเดียว</span>}
                    {p.name_shopee && !p.mark_shopee2 && <span style={{ fontSize: 10, background: '#fef08a', color: '#854d0e', padding: '2px 6px', borderRadius: 4, fontWeight: 'bold' }}>✓ SHOPEE</span>}
                    ...
                    {p.mark_thaimart && <span style={{ fontSize: 10, background: '#e0e7ff', color: '#4338ca', padding: '2px 6px', borderRadius: 4, fontWeight: 'bold' }}>✓ THAIMART</span>}
                  </div>
              ...
              <div style={{ borderTop: '1px dashed #eaeaea', paddingTop: 12, display: 'flex', flexDirection: 'column', gap: 8 }}>
                ...
                <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap', alignItems: 'center', marginTop: 4 }}>
                  ...
                  <label style={{ fontSize: 12, display: 'flex', alignItems: 'center', gap: 4 }}>
                    <input type="checkbox" checked={!!p.mark_thaimart} onChange={e => handleUpdate(p.id, 'mark_thaimart', e.target.checked)} />
                    Thaimart
                  </label>
                </div>
