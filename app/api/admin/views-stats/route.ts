...
// Convert local Bangkok midnight to UTC ISO string
// Uses Date.UTC which handles day/month overflow automatically (e.g. day=32 → next month day 1)
// Bangkok = UTC+7, so midnight BKK = hour -7 in UTC terms
...
function byDay(rows: Row[], days: number): Bucket[] {
  ...
  return Array.from({ length: days }, (_, i) => {
    ...
    const fromISO = localMidnightToUTC(y, m, day);
    const toISO   = localMidnightToUTC(y, m, day + 1);
    return { label: `${day}/${m}`, count: counts[k] ?? 0, from: fromISO, to: toISO };
  });
}
...
function byWeek(rows: Row[], weeks: number): Bucket[] {
  ...
  return Array.from({ length: weeks }, (_, i) => {
    ...
    const k = `${y}-${String(m).padStart(2,'0')}-${String(day).padStart(2,'0')}`;
    const fromISO = localMidnightToUTC(y, m, day);
    const toISO   = localMidnightToUTC(y, m, day + 7);
    return { label: `${day}/${m}`, count: counts[k] ?? 0, from: fromISO, to: toISO };
  });
}
...
function byMonth(rows: Row[], months: number): Bucket[] {
  ...
  return Array.from({ length: months }, (_, i) => {
    ...
    const fromISO = localMidnightToUTC(y, m, 1);
    // First day of next month
    const nd = new Date(d); nd.setMonth(nd.getMonth() + 1);
    const toISO = localMidnightToUTC(nd.getFullYear(), nd.getMonth() + 1, 1);
    return { label: MONTHS[m - 1], count: counts[k] ?? 0, from: fromISO, to: toISO };
  });
}
