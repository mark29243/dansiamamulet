          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderTop: '1px solid var(--cream-dark)', paddingTop: 12, marginTop: 'auto' }}>
            <div>
              <div style={{ display: 'flex', alignItems: 'baseline', gap: 6 }}>
                <span className="serif" style={{ fontSize: 18, fontWeight: 600, color: 'var(--gold-dark)' }}>{formatPrice(displayPrice, lang)}</span>
                {hasDiscount && <span style={{ fontSize: 11, color: 'var(--text-faint)', textDecoration: 'line-through' }}>{formatPrice(p.price, lang)}</span>}
              </div>
